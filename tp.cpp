...
void mostrarLista (Incidentes *lista)
{
    ...
    while (lista != NULL)
    {
        ...
        cout<<"codigo incidente: "<<lista->codigoIncidente<<endl;
        cout<<"dni: "<<lista->dniAsegurado<<endl;
        cout<<"dni otro: "<<lista->dniOtroConductor<<endl;
        cout<<"fecha hora: "<<lista->fechaHora<<endl;
        cout<<"nro poliza: "<<lista->nroPoliza<<endl<<endl;
        lista = lista->sgte;
        i++;
    }
...
int main(){
    ...
    do
    {
        ...
        case '4':
                ...
                {
                    fseek(f, 0, SEEK_END);
                    cant_alus = ftell(f)/ sizeof(Asegurados);
                    fseek(f, 0, SEEK_SET);
                    fread(valus, sizeof(Asegurados),cant_alus, f);
                    ordenar(valus, cant_alus);
                    fclose(f);
                }
                if (f = fopen("Asegurados.BAK", "wb"))
                {
                    fwrite(valus,sizeof(Asegurados), cant_alus, f);
                    fclose(f);
                }
                mostrarPolizas();
                cout << endl << "Presione un boton para continuar";
...
/*
Al comenzar la jornada se �levantan� todas las p�lizas desde el archivo �Asegurados.BAK�.
Cada vez que el usuario requiera se procesar� un �lote de incidentes�. Se desconoce cu�ntos
incidentes puede haber en cada lote, pero puede ser tan grande que haga dif�cil su manejo en
un vector.
El procesamiento de un lote de incidentes implica la actualizaci�n de la cantidad de incidentes
de aquellas p�lizas que sean afectadas. Adem�s, cada vez que se procesa un lote, deben
�transladarse� los registros procesados a un archivo llamado �procesados.BAK�, el cual
contiene todos los incidentes de todos los lotes procesados en el d�a.
Al finalizar el d�a se reescribe el archivo �Asegurados.BAK�. con las p�lizas activas �nicamente
...
2. Cargar una nueva p�liza.
3. Desactivar una p�liza existente.
4. Buscar una p�liza por Nro. de P�liza o por DNI (Un cliente puede tener m�s de una
p�liza).
5. Listar todas las polizas activas ordenadas por cantidad de incidentes en orden descendente.
6. Procesar un lote de incidentes.
7. Mostrar todas las p�lizas que no tengan la cuota al d�a. La salida debe ser en un
...
10. Se recomienda realizar un procedimiento de prueba para crear un incidente o m�s
incidentes en un archivo (para luego poder proce
*/